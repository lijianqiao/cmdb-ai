...
2. 能力类不能按单条跟基线比：repeat=5 的粒度是 0.2，模型随机翻一次就跌 0.2，
   ...
   所以看 7 条 × 5 次 = 35 次的汇总，粒度约 0.029。单条成功率照样记录，
   只用来定位是哪条退化了。
...
# 2026-08-18 实测校准后的取值（deepseek-v4-flash）。
#
# 原定的推导方式是「连跑三轮，阈值 = 最大差值 × 2」。实测三轮都是 1.000，
# **差值为 0，公式算出阈值 0**，不能用——用例集目前处在天花板上。
# 改用二项分布推导：
#   - 126 次 turn、0 次失败 → rule of three，单次失败率 95% 上界 3/126 ≈ 2.4%
#   - 每轮 47 次（2026-09-23 加了批量端口用例，原为 42 次），按此失败率期望失败约 1 次
#   - 0.10 对应「47 次里错 ≥5 次」，在该失败率下概率约 1%——够灵敏也不吵
#   - 0.05 对应「错 ≥3 次」，概率约 8%，会经常误报
#
...
def judge(
    ...
    *,
    ...
    # 全量口径同时也让样本更大：11 条用例共 47 次，粒度约 1/47 ≈ 0.021。
    # 只有跌破阈值才红；变好了不管。
    overall_pass_rate = (
