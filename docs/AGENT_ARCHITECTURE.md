...
| **T10** | HITL + 安全闸门：`app/agent/hitl.py` 状态机，`propose_remediation` 工具，`DeviceControlExecutor` 接口 + stub，新增权限码（`knowledge:*`/`cmdb:*`/`monitor:*`/`agent:hitl_approve`） | P1 + 安全基线（第 9 节） | T06 |
