...
| `query_device_command`    | `asset_id, command_name, interface_name?, ip_address?, reason`  | 只读诊断命令（只查单个接口的命令带 `interface_name`，`ping` 带 `ip_address` 目标）：按会话 `approval_mode` 判定——`assist` 且白名单+非动态凭据可当场返回输出；`full` 另可当场执行未分类非动态命令；默认 `ask` 及动态凭据走 `PENDING`。`ping` 的目标不在 CMDB 登记范围内（不是登记设备的 IP、也不落在任何设备登记的 `subnet_cidr` 内）时，不论档位都转人工审批（D2） | 读（经 HITL 门控） |
...
| L2 动作审查   | `notify.payload` 做 JSON Schema 校验，不接受自由文本命令；`device_control` 的 `command_name` 必须在设备命令目录内且通过参数校验（如 `interface_names` 逐个过接口名白名单、去重、最多 48 个；`ip_address` 必须是可探测的单播 IP，拒绝主机名与组播/广播地址），不接受自由文本 CLI；门控工具在 `HitlGateHook.before` 用与 dispatch 相同的 Pydantic 模型校验 |
