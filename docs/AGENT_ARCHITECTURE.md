...
| `query_device_command`    | `asset_id, command_name, interface_name?, ip_address?, mac_address?, vlan_id?, reason`  | 只读诊断命令（按目录登记带参数：单接口命令带 `interface_name`，`ping` / 按 IP 查表带 `ip_address`，按 MAC 查表带 `mac_address`，按 VLAN 查带 `vlan_id`；排查类命令目前只登记华为 / H3C）：按会话 `approval_mode` 判定——`assist` 且白名单+非动态凭据可当场返回输出；`full` 另可当场执行未分类非动态命令；默认 `ask` 及动态凭据走 `PENDING`。`ping` / `traceroute` 的目标不在 CMDB 登记范围内（不是登记设备的 IP、也不落在任何设备登记的 `subnet_cidr` 内）时，不论档位都转人工审批（D2）。整张 MAC / ARP / 路由 / ACL 表的输出截断到 300 行并附说明，引导改用 `*_lookup` 精确查询 | 读（经 HITL 门控） |
| `device_control`          | `asset_id, command_name, interface_names?, reason` | 变更类命令（`reboot`/`port_enable`/`port_disable`/`save_config`）：`assist` 且白名单+非动态凭据可当场执行；`full` 另可当场执行未分类非动态命令；默认 `ask` 及动态凭据 `PENDING` 待审批。端口启停一次接一组接口（最多 48 个，一条提案一次审批）；超过 8 个接口的批量不论档位都转人工审批 | 写（HITL 门控）    |
...
- **`UNKNOWN` 只留给真正不确定的失败**：执行器用 `ExecutionResult.dispatched` 区分两类失败——连接尚未建立就失败（平台/驱动不支持、认证失败、主机不可达），或者思科设备登录后用 CMDB 登记的静态 enable 口令提权失败，都说明命令确定没下发、设备状态未被改动，原子转回 `APPROVED` 并写 `status_reason=dispatch_failed_before_send`，管理员修好前置条件即可直接重试；连接建立之后的任何失败都无法确定命令是否已生效，变更命令仍走 `UNKNOWN` 人工核实。只读命令是例外：它没有副作用，重跑一次是安全的，所以连上设备后失败（设备回 `Unrecognized command`、读超时）也转回 `APPROVED` 并写 `status_reason=read_only_failed` 直接可重试——新命令模板和设备版本对不上时这种失败会成批出现，落 `UNKNOWN` 只会堆一批要人工处置的提案
- **设备回了文本不等于成功**：只有明确的成功证据才落 `EXECUTED`，判定规则登记在命令目录（`device_commands.py`）里——配置命令把按厂商登记的报错句式交给 Netmiko `error_pattern` 逐行检查，命中即失败（可能部分生效，走 `UNKNOWN`）；Junos 提交前先 `commit check`，看到 `configuration check succeeds` 才 `commit`（检查不通过就不提交，并把设备给出的错误原因带回来），提交后必须看到 `commit complete`；普通命令只检查输出开头几行的厂商报错句式（不用宽泛的 `error` 正则扫整段输出，避免把配置正文里的 logging errors 误判）；确认流程按目录逐轮匹配，设备问了没登记的问题就停下、不替人回答；重启前设备问「要不要保存配置」时同样停下（D5：保存与否是人的决定），并在原因里直接提示先执行 `save_config` 再重启；重启发出后连接会断、拿不到成功证据，一律落 `UNKNOWN` 并提示在设备恢复后人工核实（管理员在卡片上「确认已执行」即转 `EXECUTED`）。设备报错不改变 `dispatched=True`：连上设备之后的失败都按「可能已触及设备」处理
- **读超时按命令分档**：命令目录登记 `read_timeout_class`（`short` / `long`），执行器按档取 `DEVICE_COMMAND_READ_TIMEOUT_SECONDS`（默认 60）或 `DEVICE_COMMAND_LONG_READ_TIMEOUT_SECONDS`（默认 180）；`show_running_config` 这类大输出命令归 `long`，共用 60 秒的话大配置必定读超时，而读超时算"连上之后失败"。并发执行时"等别人跑完"的轮询窗口按同一档位取，否则长命令还在跑、另一个调用方已经判定等待超时
...
| L2 动作审查   | `notify.payload` 做 JSON Schema 校验，不接受自由文本命令；`device_control` 的 `command_name` 必须在设备命令目录内且通过参数校验（如 `interface_names` 逐个过接口名白名单、去重、最多 48 个；`ip_address` 必须是可探测的单播 IP，拒绝主机名与组播/广播地址；`mac_address` 接受三种常见写法、统一存成 12 位小写十六进制，渲染时按厂商格式化；`vlan_id` 为 1–4094 的整数），不接受自由文本 CLI；门控工具在 `HitlGateHook.before` 用与 dispatch 相同的 Pydantic 模型校验 |
...
| A6   | `device_control` 生产启用前的手工验证                                                                 | 已接入 Netmiko 执行通道；**生产启用 `state_changing` 命令白名单前**，须在测试网段真实/虚拟设备上手工验证 `reboot`（重启确认提示能被目录里的句式命中、保存配置的询问不会被自动回答）、`port_disable`/`port_enable`（`send_config_set` 含 Junos `commit`）行为，以及各厂商报错句式能识别语法错误/权限拒绝/接口不存在，验证记录归档后方可对生产资产创建白名单策略 |
