...
| `query_device_command`    | `asset_id, command_name, interface_name?, ip_address?, mac_address?, vlan_id?, reason`  | 只读诊断命令（按目录登记带参数：单接口命令带 `interface_name`，`ping` / 按 IP 查表带 `ip_address`，按 MAC 查表带 `mac_address`，按 VLAN 查带 `vlan_id`；排查类命令目前只登记华为 / H3C）：按会话 `approval_mode` 判定——`assist` 且白名单+非动态凭据可当场返回输出；`full` 另可当场执行未分类非动态命令；默认 `ask` 及动态凭据走 `PENDING`。`ping` 的目标不在 CMDB 登记范围内（不是登记设备的 IP、也不落在任何设备登记的 `subnet_cidr` 内）时，不论档位都转人工审批（D2） | 读（经 HITL 门控） |
