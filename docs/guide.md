...
- 当前会话所有者可在已执行的 `device_query` 卡片点击“查看完整配置”；前端才按需请求该会话、该提案的专用完整结果。其它用户或其它会话无法借由更换 ID 读取。
- 若 AI 总结失败，设备查询仍为 `EXECUTED`，对话会显示“设备配置已成功获取，但 AI 总结生成失败。请在审批卡片中展开查看完整原始配置。”；用户仍可读取完整配置。
