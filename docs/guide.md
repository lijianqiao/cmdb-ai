...
硬规则：只有 `PENDING` 可审批决定；`REJECTED` / `EXECUTED` 为终态；认领 `EXECUTING` 前须策略复检且事务先提交，外部执行器才启动；命令不存在或动态凭据缺失时不认领、状态保持 `APPROVED`；当前策略已黑名单时原子转 `REJECTED` 并写 `status_reason=policy_blacklisted`；`UNKNOWN` 不自动重试（人工处置见 §5.3.2）；待审批时敏感结果**不回传模型**；落盘原子写。

#### 5.3.1 动态凭据设备查询：审批后的结果与恢复（本项目）

动态凭据设备查询在审批卡片中输入的本次密码只用于当前执行调用，不会保存或回传模型。批准成功后，系统先把设备完整原始输出保存为专用结果，再保留最多 4000 字符的安全预览；随后同步生成无工具 AI 总结，并将总结写成一条普通 assistant 消息，因此用户无需再追问一次。

- 审批卡片和刷新后的会话快照只展示安全预览与结果入口，绝不通过 WebSocket、快照、审计或普通对话历史发送完整正文。
- 当前会话所有者可在已执行的 `device_query` 卡片点击“查看完整输出”；前端才按需请求该会话、该提案的专用完整结果。其它用户或其它会话无法借由更换 ID 读取。
- 若 AI 总结失败，设备查询仍为 `EXECUTED`，对话会显示“设备命令已执行成功，但 AI 总结生成失败。请在审批卡片中展开查看完整原始输出。”；用户仍可读取完整输出。
- AI 总结按「这条命令是查什么的」来提炼（命令用途取自命令目录的说明）：查 ARP 看查到的条目和接口，接口类看状态和错包，整份配置才按配置审阅的口径总结。
- 输出能按 TextFSM 模板（Netmiko 自带的 ntc-templates）解析时，逐行字段会附在原文后面一起交给总结模型，便于逐条核对；解析是用原文现场做的，不另存，模板对不上就只给原文，与原文冲突时以原文为准。
- 若结果显示总结待处理，或生成中的认领已过期，卡片会显示“恢复 AI 总结”。此按钮只对已经保存的完整正文重新生成总结，**不会**重新连接设备、再次执行命令或要求输入动态密码。

自动批准的查询仍由现有 Agent loop 基于安全预览回答；它不会额外走人工审批后的同步总结编排。

#### 5.3.2 管理员处置 UNKNOWN 提案（本项目）
