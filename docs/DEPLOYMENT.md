...
审批证据保留迁移（`e3a7c1f9b246`）把 `hitl_proposals.session_id` 外键从级联删除改为
`RESTRICT`，新增申请人、审批方式和证据快照三列，并按会话所有者与审计日志回填能确认的
申请人和审批方式。重建外键会校验全表，期间 `hitl_proposals`/`agent_sessions` 的写入
会短暂等待，提案量大时同样安排在短只读窗口。升级前已被级联删掉的提案迁移找不回来，
只能从备份恢复；历史提案的当时快照保持为空，不用今天的资产值冒充。升级后，关联审批
证据的用户不能再永久删除（接口返回 409），可以留在回收站。
