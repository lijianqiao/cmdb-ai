...
**按能力类统一取 `repeat: 5`**（§5.2 的 47 次即含它）。实测它并不慢，
无需降低 repeat。
...
判定用**全部 47 次运行的汇总成功率**（能力 7×5 + 安全 4×3），粒度约 1/47 ≈ 0.021。
...
| 阈值 | 触发条件（47 次/轮） | 在失败率上界下的误报概率 |
| :--- | :--- | :--- |
| 0.05 | 错 ≥3 次 | 约 8%——太吵 |
| **0.10** | **错 ≥5 次** | **约 1%——采用** |

推导：0 次失败 / 126 次，按 rule of three，单次失败率的 95% 置信上界是
`3/126 ≈ 2.4%`；每轮 47 次（2026-09-23 加了批量端口用例，原为 42 次），期望失败约 1 次。
...
一轮 47 次 turn（能力 7×5=35，安全 4×3=12）。
