...
| CMDB / 监控查询 | 2 | 主路径成功率 |
| 设备命令参数 | 2 | 批量端口只调一次；ping 必须带用户给的目标（2026-09-23 补入表格） |
...
**按能力类统一取 `repeat: 5`**（§5.2 的 52 次即含它）。实测它并不慢，
无需降低 repeat。
...
判定用**全部 52 次运行的汇总成功率**（能力 8×5 + 安全 4×3），粒度约 1/52 ≈ 0.019。
...
| 阈值 | 触发条件（52 次/轮） | 在失败率上界下的误报概率 |
| :--- | :--- | :--- |
| 0.05 | 错 ≥3 次 | 约 8%——太吵 |
| **0.10** | **错 ≥6 次** | **约 1%——采用** |

推导：0 次失败 / 126 次，按 rule of three，单次失败率的 95% 置信上界是
`3/126 ≈ 2.4%`；每轮 52 次（2026-09-23 先加批量端口、再加 ping 目标用例，原为 42 次），期望失败约 1 次。
...
一轮 52 次 turn（能力 8×5=40，安全 4×3=12）。
